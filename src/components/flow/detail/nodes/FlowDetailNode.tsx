...
export type FlowDetailNodeData = {
  note: { id: number };
  size: number;
...
export default function FlowDetailNode({
  data,
  selected,
}: NodeProps<FlowDetailNodeData>) {
  const montages = data.montages ?? [];
  const montageCount = montages.length;
  const nodeDiameter = Math.max(0, Number(data.size) || 0);
  const baseRadius = Math.max(80, nodeDiameter / 2 + 32);
  ...
  return (
    <div className={styles.nodeContent}>
      ...
      <div className={styles.nodeLabel}>
        <span className={styles.nodeTitle}>{data.labelText}</span>
        <span
          className={`${styles.nodeMeta} ${data.emphasizeDate ? styles.nodeMetaDateEmphasis : ""}`.trim()}
        >
          {data.showNoteId === false ? null : <>{`#${data.note.id}`}</>}
          {data.dateText ? (
            <>
              {data.showNoteId === false ? null : " · "}
              {data.dateText}
            </>
          ) : null}
        </span>
