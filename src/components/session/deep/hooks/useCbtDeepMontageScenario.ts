...
export function useCbtDeepMontageScenario(
  ...
  const query = useQuery({
    ...
    queryFn: async () => {
      ...
      if (isDev) {
        console.info("[deep/montage-scenario] query:start", {
          key,
      ...
      try {
        ...
        const result = await createDeepMontageScenario(mainContext, subContexts);
        if (isDev) {
          console.info("[deep/montage-scenario] query:success", {
            key,
